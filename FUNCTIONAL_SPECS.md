...
- Mount camera with a clear view of the field.
...
1. In the PhotonVision UI, go to the **Settings** tab.
...
1. Create a new pipeline: click the **+** button.
...
1. Download calibration board:
	...
	- Print on a flat surface (foam board recommended).
2. Run calibration in the PhotonVision UI:
...
Ensure the robot can reach the coprocessor:

- **mDNS**: `photonvision.local` (should work automatically).
- **Static IP**: `10.TE.AM.11` (if mDNS fails). Configure in PhotonVision Settings → Networking.
...
1. Drive robot to a **known position** on the field.
2. Reset odometry to that position.
3. Observe the `Vision Field` widget on SmartDashboard.
4. Vision estimate should be close to odometry (within ~0.5 m initially).
...
- Watch `Vision/Best Target Ambiguity` on the dashboard.
...
- [ ] Pose estimates are reasonable (< 0.5 m error)
...
3. **Tag Coverage Test**:
	...
	- Verify at least one tag is visible from most locations.
...
- **Source/Loading Zones**: Specific tags vary, check the game manual.
...
- Try a different USB port on the coprocessor.
...
- Verify the computer is on the robot network.
- Try the IP address instead of mDNS: `http://10.TE.AM.11:5800`.
...
- Recalibrate the camera (most common issue).
...
- Use a higher resolution camera or setting.
...
Important placeholders to configure:
